...
@section('content')
    ...
    <div class="admin-container">
        <div style="background: #FFFFFF; border-radius: 16px; padding: 32px; box-shadow: 0 4px 16px rgba(0,0,0,0.1); max-width: 800px;">
            <h1 style="color: #1C6EA4; margin-bottom: 24px;">Add New Grade</h1>
            ...
            <form method="POST" action="{{ route('admin.grades.store') }}">
                    ...
                    <div>
                        ...
                        <select name="user_id" required style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                            ...
                            @foreach($students as $student)
                                <option value="{{ $student->id }}" {{ old('user_id') == $student->id ? 'selected' : '' }}>{{ $student->name }} ({{ $student->student_id }})</option>
                            @endforeach
                        </select>
                        @error('user_id')
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 16px;">
                    <div>
                        ...
                        <input type="text" name="semester" required value="{{ old('semester') }}" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        @error('semester')
                            <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="academic_year" required value="{{ old('academic_year') }}" placeholder="e.g., 2024-2025" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                        @error('academic_year')
                            <p style="color: #EF4444; font-size: 0.875rem; margin-top: 4px;">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px;">
                    <div>
                        ...
                        <input type="text" name="midterm" value="{{ old('midterm') }}" placeholder="e.g., 85.5 or INC" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                    </div>
                    <div>
                        <label style="display: block; margin-bottom: 6px; color: #374151; font-weight: 500;">Finals</label>
                        <input type="text" name="final" value="{{ old('final') }}" placeholder="e.g., 90.0 or INC" style="width: 100%; padding: 10px; border: 1px solid #D1D5DB; border-radius: 6px;">
                    </div>
                </div>
                ...
                <div style="display: flex; gap: 12px;">
                    ...
                    <button type="submit" style="padding: 10px 20px; background: #1C6EA4; color: white; border: none; border-radius: 6px; cursor: pointer;">Create Grade</button>
                </div>
            </form>
        </div>
    </div>
