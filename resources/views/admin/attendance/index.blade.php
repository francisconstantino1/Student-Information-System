...
@section('content')
    ...
    <div class="admin-container">
        ...
        <div class="grid-2">
            ...
            <div class="card">
                ...
                <form method="POST" action="{{ route('admin.attendance.generate-code') }}">
                    @csrf
                    <div class="form-group">
                        ...
                        <select name="class_session_id" id="class_session_id" class="form-select" required>
                            ...
                            @foreach($classSessions as $session)
                                <option value="{{ $session->id }}">{{ $session->name }} ({{ $session->time_range }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                            ...
                            <div style="padding: 12px; background: #F9FAFB; border-radius: 8px; border: 1px solid #E5E7EB;">
                                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                    ...
                                    @if($code->is_active && !$code->isExpired())
                                        <form method="POST" action="{{ route('admin.attendance.deactivate-code', $code->id) }}" style="display: inline;">
                                            @csrf
                                ...
                                <div style="font-size: 0.75rem; color: #6B7280;">
                                    <div><strong>Session:</strong> {{ $code->classSession->name ?? 'N/A' }} @if($code->classSession)({{ $code->classSession->time_range }})@endif</div>
                                    <div><strong>Date:</strong> {{ $code->date->format('M d, Y') }}</div>
                                    <div><strong>Expires:</strong> {{ $code->expires_at->format('M d, Y g:i A') }}</div>
        ...
        @if($attendances->total() > 0)
            <div class="card">
                ...
                <p style="color: #6B7280; font-size: 0.875rem; margin-bottom: 16px;">Student attendance submissions</p>
                ...
                <table id="allCodesTable" class="display" style="width:100%">
                    ...
                    <tbody>
                        @foreach($allCodes as $code)
                            <tr>
                                <td>
                                    <span class="code-badge {{ $code->isExpired() || !$code->is_active ? 'code-expired' : 'code-active' }}">
                                        {{ $code->code }}
                                    </span>
                                </td>
                                <td>{{ $code->classSession->name ?? 'N/A' }} @if($code->classSession)({{ $code->classSession->time_range }})@endif</td>
                                <td>{{ $code->date->format('M d, Y') }}</td>
                                <td>{{ $code->expires_at->format('M d, Y g:i A') }}</td>
                                <td>
                                    @if($code->isExpired())
    ...
    @foreach($attendances as $attendance)
        <div class="modal" id="view-attendance-modal-{{ $attendance->id }}">
            <div class="modal-content">
                    ...
                    <div class="form-row">
                        <div class="form-col">
                            <div class="form-group">
                                ...
                                <div class="view-field">{{ $attendance->user->student_id ?? 'N/A' }}</div>
                            </div>
                        </div>
                        <div class="form-col">
                            <div class="form-group">
                                <label>Student Name</label>
                                <div class="view-field">{{ $attendance->user->name }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="form-section">
                    <h5><i class="fas fa-calendar-check"></i> Attendance Information</h5>
                    <div class="form-row">
                        <div class="form-col">
                            <div class="form-group">
                                <label>Date</label>
                                <div class="view-field">{{ $attendance->date->format('F j, Y') }}</div>
                            </div>
                        </div>
                        <div class="form-col">
                            <div class="form-group">
                                <label>Time In</label>
                                <div class="view-field">{{ $attendance->time_in ?? 'N/A' }}</div>
                            </div>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-col">
                            <div class="form-group">
                                ...
                                <div class="view-field">{{ $attendance->attendanceCode && $attendance->attendanceCode->classSession ? $attendance->attendanceCode->classSession->time_range : 'N/A' }}</div>
                            </div>
                        </div>
                        <div class="form-col">
                            <div class="form-group">
                ...
                <div class="modal-buttons">
                    <button type="button" class="btn exit-btn" onclick="closeModal('view-attendance-modal-{{ $attendance->id }}')">
